...
module Monads.CatofAdj.InitAdj {C}(M : Monad C) where
...
lemfid : ∀{D}(L : Fun C D)(R : Fun D C)(T : Fun C C)(p : T ≅ R ○ L)
  ...
  {X : Obj C} → 
  right (subst (Hom C X) (fcong X (cong OMap p)) η) ≅ iden D {OMap L X}
lemfid {D} L R .(R ○ L) refl η right left q r {X} = 
  trans (cong right (sym r)) (q (iden D))

lemfcomp : ∀{D}(L : Fun C D)(R : Fun D C)(T : Fun C C)(p : T ≅ R ○ L)
...
lemfcomp {D} L R .(R ○ L) refl bind right q r {X}{Y}{Z}{f}{g} = 
...
lemLlaw : ∀{D}(L : Fun C D)(R : Fun D C)(T : Fun C C)(p : T ≅ R ○ L)
  (η : ∀ {X} → Hom C X (OMap T X)) → 
  (right : {X : Obj C} {Y : Obj D} → Hom C X (OMap R Y) → Hom D (OMap L X) Y) →
  (left : {X : Obj C} {Y : Obj D} → Hom D (OMap L X) Y → Hom C X (OMap R Y)) → 
  (q : {X : Obj C} {Y : Obj D} (f : Hom D (OMap L X) Y) → right (left f) ≅ f) → 
...
lemLlaw {D} L R .(R ○ L) refl η right left q r t {X}{Y} f = trans 
   (cong (λ g → right (comp C g f)) (sym r)) 
   (trans 
     (trans 
       (trans 
         (cong 
           ...
           (trans 
             (sym (idl C)) 
             (cong (λ g → comp C g (comp C (left (iden D)) f)) (sym (fid R)))))
...
rightlaw' A {X}{Y}{f} = 
  cong 
    (right (adj A))
    (trans 
      (stripsubst (Hom C X) f (fcong Y (cong OMap (sym (law A)))))
      (sym
  ...
  (FunctorEq 
    ...
    (λ {A} {B} f → trans 
      (cong₂ 
        ...
        (trans 
          (stripsubst (Hom C A) f (fcong B (cong OMap (sym (law X)))))
