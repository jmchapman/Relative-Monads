...
lemX : ∀{a b}{C : Cat {a}{b} }(M : Monad C) → R (KlAdj M) ○ L (KlAdj M) ≅ TFun M
lemX {C} M = FunctorEq _ _ refl (λ f → refl) 

KlObj : ∀{a b}{C : Cat {a}{b}}(M : Monad C) → 
        Obj (CatofAdj M)
KlObj {C = C} M = record { 
  D   = Kl M; 
  adj = KlAdj M; 
...
lemZ : ∀{a b}{C D : Cat {a}{b}}{F G G' : Fun C D} → 
       G ≅ G' → {α : NatT F G}{β : NatT F G'} → α ≅ β → 
       ∀ {X} → cmp α {X} ≅ cmp β {X}
lemZ refl refl = refl

lemZ' : ∀{a b}{C D : Cat {a}{b}}{F F' G G' : Fun C D} → 
        F ≅ F' → G ≅ G' → {α : NatT F G}{β : NatT F' G'} → α ≅ β → 
        ∀ {X} → cmp α {X} ≅ cmp β {X}
lemZ' refl refl refl = refl


lemfid : ∀{a b}{C D : Cat {a}{b}}(L : Fun C D)(R : Fun D C)(T : Fun C C) 
  ...
  {X : Obj C} → 
    ...
    ≅ 
    iden D {OMap L X}
lemfid {C = C}{D = D} L R .(R ○ L) refl η right left q r {X} = 
  trans (cong right (sym r)) (q (iden D))

lemfcomp : ∀{a b}{C D : Cat {a}{b}}(L : Fun C D)(R : Fun D C)(T : Fun C C) 
  (p : T ≅ R ○ L)
...
lemfcomp {C = C}{D = D} L R .(R ○ L) refl bind right q r {X}{Y}{Z}{f}{g} = 
...
lemLlaw : ∀{a b}{C D : Cat {a}{b}}(L : Fun C D)(R : Fun D C)(T : Fun C C) 
          (p : T ≅ R ○ L) (η : ∀ {X} → Hom C X (OMap T X)) → 
          ...
          ≅ 
          HMap L f
lemLlaw {C = C}{D = D} L R .(R ○ L) refl η right left q r t {X}{Y} f = 
...
K' : ∀{a b}{C : Cat {a}{b}}(M : Monad C)(A : Obj (CatofAdj M)) → 
     Fun (D (KlObj M)) (D A)
K' {C = C} M A = record {
                 OMap = OMap (L (adj A));
                 HMap =
...
Llaw' : ∀{a b}{C D : Cat {a}{b}}(M : Monad C)(A : Obj (CatofAdj M)) → 
        K' M A ○ L (adj (KlObj M)) ≅ L (adj A)
Llaw' {C = C} M A = FunctorEq 
  _ 
  _ 
  refl
  (lemLlaw (L (adj A)) 
...
Rlaw' : ∀{a b}{C : Cat {a}{b}}(M : Monad C)(A : Obj (CatofAdj M)) → 
        R (adj (KlObj M)) ≅ R (adj A) ○ K' M A
Rlaw' {C = C} M A = FunctorEq 
  _ 
  _ 
  (cong OMap (sym (law A))) (λ f → sym (bindlaw A))

rightlaw' : ∀{a b}{C : Cat {a}{b}}(M : Monad C)(A : Obj (CatofAdj M)) → 
...
KlHom : ∀{a b}{C : Cat {a}{b}}(M : Monad C){A : Obj (CatofAdj M)} → 
        Hom (CatofAdj M) (KlObj M) A
KlHom {C = C} M {A} = record { 
  K        = K' M A; 
  Llaw     = Llaw' M A; 
  Rlaw     = Rlaw' M A; 
...
KlIsInit : ∀{a b}{C : Cat {a}{b}}(M : Monad C) → Init (CatofAdj M)
KlIsInit {C = C} M = record { 
  I   = KlObj M;
  i   = KlHom M;
  law = uniq M}
