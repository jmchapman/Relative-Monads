...
delayMonad : ∀ {i} → RawMonad {f = lzero} (λ A → Delay i A)
delayMonad {i} = record
  { return = now
  ; _>>=_  = _>>=_ {i}
  } where open Bind
...
{-
mutual
  _⇓_ : {A : Set} {i : Size} (x : Delay i A) (a : A) → Set
  x ⇓ a = Terminates a x

  data Terminates {A : Set} {i : Size} (a : A) : Delay i A → Set where
    now   : now a ⇓ a
    later : ∀ {x : ∞Delay i A} → (force x {j = ?}) ⇓ a → later x ⇓ a

mutual

  cast : ∀ {A i} → Delay i A → (j : Size< ↑ i) → Delay A j
  cast (now x) j = now x
