...
lem3 : ∀ A → {B C : ℕ} (f : Fin A → Fin C) (g : Fin B → Fin C)
  (h : Fin (A + B) → Fin C) →
  (λ x → h (extend {A} x)) ≅ f →
  (λ x → h (lift A x)) ≅ g → ∀ i → h i ≅ split A f g i
