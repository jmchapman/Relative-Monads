...
resp f refl = refl
-}
respid : ∀{A}{a a' : A}(p : a ≅ a') → cong id p ≅ p
respid refl = refl
...
icong : ∀{A}{B : A → Set}(f : ∀ {a} → B a){a a' : A} → a ≅ a' → f {a} ≅ f {a'}
icong f refl = refl
...
dcong : ∀{A A' B B'}{f : A → B}{f' : A' → B'}{a : A}{a' : A'} → a ≅ a' → B ≅ B' → f ≅ f' → f a ≅ f' a'
...
ifresp : ∀{A}{B : A → Set}{f f' : {x : A} → B x}(a : A) → 
         _≅_ {_}{ {x : A} → B x} f { {x : A} → B x} f' → f {a} ≅ f' {a}
ifresp a refl = refl

funnycong : ∀{A}{B : A → Set}{C : Set}{a a' : A} → a ≅ a' → {b : B a}{b' : B a'} → b ≅ b' → 
...
funnycong4 : {A : Set}{B : A → Set}{C : (a : A) → B a → Set}{D : (a : A)(b : B a) → C a b  → Set}{E : Set}
...
funnycong4' : {A : Set}{B : A → Set}{C : (a : A) → B a → Set}{D : (a : A) → B a → Set}{E : Set}
             {a a' : A} → a ≅ a' → 
             {b : B a}{b' : B a'} → b ≅ b' → 
             {c : C a b}{c' : C a' b'} → c ≅ c' → 
             {d : D a b}{d' : D a' b'} → d ≅ d' → 
